...
import exceptions.wedontattackexeption;
...
public class virtualgame implements Cloneable {
	...
	public int getdistance(Champion c, Champion cc) {

		int x = (int) c.getLocation().getX();
		int y = (int) c.getLocation().getY();
		int xx = (int) cc.getLocation().getX();
		int yy = (int) cc.getLocation().getY();

		int distance = Math.abs(xx - x) + Math.abs(yy - y);
		return distance;
	}

	public int getdictancefromcurrenttoenemies() {
		int distance = 0;
		ArrayList<Champion> champions = null;
		if (firstPlayer.getTeam().contains(this.getCurrentChampion())) {
			champions = secondPlayer.getTeam();
			// System.out.println("we choose from first");
		} else {
			champions = firstPlayer.getTeam();
			// System.out.println("we choose from second");
		} // System.out.println("FIRSTPLAYERTEAM");
			// for(Champion c:firstPlayer.getTeam())
			// System.out.println(c.getName());
		// System.out.println("enemy'steam :"+champions);
		for (Champion c : champions) {
			distance += getdistance(getCurrentChampion(), c);
			// System.out.println("distancefrom "+c.getName()+"
			// "+getdistance(getCurrentChampion(), c));
		}
		return distance;
	...
	public void printgrid() {
		for (Object[] o : getBoard()) {
			for (Object oo : o)
				if (oo instanceof Champion)
					System.out.print(((Champion) oo).getName()+"    ");
				else
					System.out.print(oo+"     ");
			System.out.println();
		}
	...
	public virtualgame(virtualgame game) {
		if (game != null) {
			firstPlayer = game.getFirstPlayer().virtualPlayer();
			secondPlayer = game.getSecondPlayer().virtualPlayer();
			availableChampions = new ArrayList<Champion>();
			availableAbilities = new ArrayList<Ability>();
			board = new Object[BOARDWIDTH][BOARDHEIGHT];
			for (int i = 0; i < 5; i++)
				for (int j = 0; j < 5; j++) {
					Object object = game.getBoard()[i][j];
					if (object instanceof Cover || object == null)
						board[i][j] = object;
					else
						board[i][j] = getcrosChampion(((Champion) object).getName());
				}

			turnOrder = game.turnOrder.getvirsual(firstPlayer, secondPlayer);
			// System.out.println("in virtualgame
			// "+((Champion)turnOrder.peekMin()).getName());

		}
	}

	public Champion getcrosChampion(String s) {
		for (Champion c : firstPlayer.getTeam())
	...
	public void move(Direction d) throws NotEnoughResourcesException, UnallowedMovementException {
		...
		// System.out.println("Currentaction point
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH) {
				...
				if (board[currx][curry] instanceof Cover) {
                    ...
                    System.out.println(getCurrentChampion().getName()+" attack "+((Champion)target).getName());
	...
	public void attackvirtual(Direction d) throws NotEnoughResourcesException, ChampionDisarmedException,
			InvalidTargetException, wedontattackexeption {
		if (hasEffect(getCurrentChampion(), "Disarm"))
			throw new ChampionDisarmedException("Can not attack while being disarmed");
		if (getCurrentChampion().getCurrentActionPoints() < 2)
			throw new NotEnoughResourcesException("You need at least two action point to perform a normal attack");
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			if (d == Direction.DOWN)
				...
				curry++;
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH) {
				throw new wedontattackexeption();
			} else if (board[currx][curry] != null) {
				if (board[currx][curry] instanceof Cover) {
					int curhp = ((Cover) board[currx][curry]).getCurrentHP();
					curhp -= getCurrentChampion().getAttackDamage();
	...
	/**
	 * @param availableChampions the availableChampions to set
	 */
	public static void setAvailableChampions(ArrayList<Champion> availableChampions) {
		virtualgame.availableChampions = availableChampions;
	}

	/**
	 * @param availableAbilities the availableAbilities to set
	 */
	public static void setAvailableAbilities(ArrayList<Ability> availableAbilities) {
		virtualgame.availableAbilities = availableAbilities;
	}
	...
	/**
	 * @param firstLeaderAbilityUsed the firstLeaderAbilityUsed to set
	 */
	public void setFirstLeaderAbilityUsed(boolean firstLeaderAbilityUsed) {
		this.firstLeaderAbilityUsed = firstLeaderAbilityUsed;
	}

	/**
	 * @param secondLeaderAbilityUsed the secondLeaderAbilityUsed to set
	 */
	public void setSecondLeaderAbilityUsed(boolean secondLeaderAbilityUsed) {
		this.secondLeaderAbilityUsed = secondLeaderAbilityUsed;
	}
