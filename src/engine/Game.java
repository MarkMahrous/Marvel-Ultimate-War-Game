...
public class Game implements Cloneable{
    ...
    public virtualgame turngametoVirtualgame() {
    	virtualgame virtualgame=new virtualgame(null);
    	Player vfirstPlayer = this.getFirstPlayer().virtualPlayer();
		Player vsecondPlayer = this.getSecondPlayer().virtualPlayer();
		virtualgame.setFirstPlayer(vfirstPlayer);
		virtualgame.setSecondPlayer(vsecondPlayer);
		Object[][] vboard = new Object[BOARDWIDTH][BOARDHEIGHT];
		for(int i=0;i<5;i++)
			for(int j=0;j<5;j++) {
				Object object=this.getBoard()[i][j];
				if(object == null)
					vboard[i][j]=object;
				if(object instanceof Champion)
				   vboard[i][j]=getcrosChampion(((Champion)object).getName(),vfirstPlayer,vsecondPlayer);
				if(object instanceof Cover)
					vboard[i][j]=((Cover)object).getvirtual();
			}
		
		
		virtualgame.setBoard(vboard);
//		PriorityQueue vPriorityQueue=new PriorityQueue(6);
//		for(Champion c:firstPlayer.getTeam())
//			vPriorityQueue.insert(c);
//		for(Champion c:secondPlayer.getTeam())
//			vPriorityQueue.insert(c);
//		int dif=vPriorityQueue.size()-this.getTurnOrder().size();
//		while(dif-->=0)
//			vPriorityQueue.remove();
		virtualgame.setTurnOrder(getTurnOrder().getvirsual(vfirstPlayer,vsecondPlayer));
	return virtualgame;

    }
    public Champion getcrosChampion(String s,Player vfirstPlayer,Player vsecondPlayer) {
    	for(Champion c:vfirstPlayer.getTeam())
    		if(c.getName().equals(s))
    			return c;
    	for(Champion c:vsecondPlayer.getTeam())
    		if(c.getName().equals(s))
    			return c;
    	return null;
    }
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					if (curhp <= 0) {
						listener.death((Cover)board[currx][curry], currx, curry);	
						board[currx][curry] = null;
				
					}
					listener.attack(d,(Damageable)board[currx][curry]);
					return;
		...
		if (a.getCastArea() == AreaOfEffect.SELFTARGET) {
			...
			if (a instanceof DamagingAbility || (a instanceof CrowdControlAbility
				...
				if (firstPlayer.getTeam().contains(getCurrentChampion()))
					...
					listener.castteamtarget(secondPlayer, false);
					listener.leaderabilityforvalain(team);

					}else
					{team = firstPlayer.getTeam();
					listener.castteamtarget(firstPlayer, false);
					listener.leaderabilityforvalain(team);
					}
				...
				if (firstPlayer.getTeam().contains(getCurrentChampion()))
					{team = firstPlayer.getTeam();
					listener.castteamtarget(firstPlayer, true);
					}else {
					team = secondPlayer.getTeam();
					listener.castteamtarget(secondPlayer, true);
					}
				listener.casthealing(team);
			}
			for (Champion c : team) {
			...
			possiblePoints.add(new Point(currx - 1, curry - 1));
			possiblePoints.add(new Point(currx - 1, curry + 1));
			listener.sroundtarget(getCurrentChampion());
	...
	public void castAbility(Ability a, int x, int y) throws NotEnoughResourcesException, AbilityUseException,
		...
		if(a instanceof HealingAbility||a instanceof CrowdControlAbility&&((CrowdControlAbility)a).getEffect().getType()==EffectType.BUFF)
			...
			listener.leaderabilityforvalaind(targets);
		}
		a.execute(targets);
		getCurrentChampion().setMana(getCurrentChampion().getMana() - a.getManaCost());
		getCurrentChampion()
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		if (getCurrentChampion() instanceof Hero) {
			...
			listener.leaderabilityforvalain(enemies);
		}
	...
	private void cleanup(ArrayList<Damageable> targets) {
		for (Damageable c : targets) {
			if (c.getCurrentHP() == 0) {
				this.listener.death(c, c.getLocation().x, c.getLocation().y);
